...
app.use('/updateConfigs.json', function (req, res, next) {
	...
	connection.query("call updateConfig(?,?,?)", params, function (err, resultsData, fields) {
		if (err) {
			throw err;
		}
	});
...
app.use('/tankUpdate.json', function (req, res, next) {
	...
	for (var i = 0; i < req.body.length; i++) {
		...
		if (req.body[i].delete  == true) {
			...
			connection.query("DELETE FROM tanques WHERE id = ?;", selParams, function (err, resultsData, fields) {
				if (err) {
					throw err;
				}
			});
			...
			connection.query("INSERT INTO tanques(code,descripcion,temp_calibration) VALUES(\'\',\'\',0);", function (err, resultsData, fields) {
				if (err) {
					throw err;
				}
			});
			...
			connection.query("UPDATE tanques SET code = ?, descripcion = ?, temp_calibration = ? where id = ?;", selParams, function (err, resultsData, fields) {
				if (err) {
					throw err;
				}
			});
...
app.use('/profUpdateSingle.json', function (req, res, next) {

	var connection = mysql.createConnection(mysqlconfig);
	console.log(req.body);
	var selParams = [];
	if (req.body.delete  == true) {
		selParams = selParams.concat([req.body.id]);
		connection.query("DELETE FROM profiles WHERE id = ?;", selParams, function (err, resultsData, fields) {
			if (err) {
				throw err;
			}
		});

	} else if (req.body.insert == true) {
		connection.query("INSERT INTO profiles(nombre,duration) VALUES(\'Nuevo\',0);", function (err, resultsData, fields) {
			if (err) {
				throw err;
			}
		});
	} else {
		selParams = selParams.concat([req.body.nombre, req.body.duration, req.body.id]);
		connection.query("UPDATE profiles SET nombre = ?, duration = ? where id = ?;", selParams, function (err, resultsData, fields) {
			if (err) {
				throw err;
			}
		});
	}
	for (var ii = 0; ii < req.body.mapa.length; ii++) {
		var selMapaParams = [];
		if (req.body.mapa[ii].delete  == true) {
			selMapaParams = selMapaParams.concat([req.body.mapa[ii].id]);
			connection.query("DELETE FROM mapatemp where id = ?;", selMapaParams, function (err, resultsData, fields) {
				if (err) {
					throw err;
				}
			});
		} else if (req.body.mapa[ii].insert == true) {
			selMapaParams = selMapaParams.concat([req.body.id]);
			connection.query("INSERT INTO mapatemp (tempFrom, tempTo, temp, tolerancia, profile) values(0,0,0,0,?);", selMapaParams, function (err, resultsData, fields) {
				if (err) {
					throw err;
				}
			});
		} else {
			selMapaParams = selMapaParams.concat([req.body.mapa[ii].tempFrom, req.body.mapa[ii].tempTo, req.body.mapa[ii].temp, req.body.mapa[ii].tolerancia, req.body.mapa[ii].id]);
			connection.query("UPDATE mapatemp SET tempFrom = ?, tempTo = ?, temp = ?, tolerancia = ? where id = ?;", selMapaParams, function (err, resultsData, fields) {
				if (err) {
					throw err;
				}
			});
	...
	connection.query("select f.id as id, f.nombre as nombre_fermentacion ,p.nombre as profile, f.fecha_inicio as fecha_inicio, getHours(f.fecha_inicio) as total_hours, f.activo as activo, p.duration as duration, t.code as tanque_code, getLastTemp(f.id) as currentTemp, getProgTemp(f.id) as progTemp, getTempPromedio(f.id) as promTemp, t.descripcion as tanque_descripcion, f.notas from fermentadores as f INNER JOIN profiles as p on f.profile = p.id INNER JOIN tanques as t on t.id = f.tanque where f.id = ?;", selParams, function (err, resultsData, fields) {
		...
		if (resultsData.length > 0) {
			var ferm = {
				...
				total_hours: parseHour(resultsData[0].total_hours <= resultsData[0].duration ? resultsData[0].total_hours : resultsData[0].duration),
...
app.use('/getFermData.json', function (req, res, next) {
	...
	var connection = mysql.createConnection(mysqlconfig);
	connection.query("select f.id as id, f.nombre as nombre_fermentacion ,p.nombre as profile , f.fecha_inicio as fecha_inicio, getHours(f.fecha_inicio) as total_hours, f.activo as activo, p.duration as duration, t.code as tanque_code, getLastTemp(f.id) as currentTemp, getProgTemp(f.id) as progTemp, getTempPromedio(f.id) as promTemp, t.descripcion as tanque_descripcion, t.status as tank_status, f.alerta, a.nombre as alerta_name, a.descripcion as alerta_desc, getLastUpdate(f.id) as lastupdate  FROM fermentadores as f INNER JOIN profiles as p on f.profile = p.id INNER JOIN tanques as t on t.id = f.tanque LEFT JOIN alertas as a on a.id = f.alerta where f.activo = ?;", selParams, function (err, resultsData, fields) {
		if (err) {
			//throw err;
			console.log('ERROR SQL');
			console.log(err);
