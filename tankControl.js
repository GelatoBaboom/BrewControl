...
setInterval(function () {
	if (readyToSerialWrite) {
		...
		connection.query('SELECT f.id as id, f.nombre as nombre, getLastUpdate(f.id) as lastupdate,getHours(f.fecha_inicio) as total_hours, p.duration as duration FROM fermentadores as f INNER JOIN profiles as p on f.profile = p.id WHERE activo = 1;', function (err, results, fields) {
			for (var i = 0; i < results.length; i++) {
				var lastupdate = results[i].lastupdate;
				var totalhours = results[i].total_hours;
				duration = results[i].duration;
				if (totalhours < duration) {

					if (results[i].lastupdate != null) {
						var minutes = moment().diff(results[i].lastupdate, 'minutes');

						if (minutes > 5) {
							console.log("El sistema no responde: reiniciando..." + minutes);
							if (port != null) {
								port.close();
								readyToSerialWrite = false;
								initializePort();
								break;
							}
						}
	...
	if (obj.f.startsWith('tank')) {
		...
		var selTempProgQry = "SELECT getProgTemp(getFermentadorFormTanqueCurrent(?)) as temp, getProgTempTolerancia(getFermentadorFormTanqueCurrent(?)) as tolerancia, getTempCalibration(?) as cal, getAlertaByFermentador(getFermentadorFormTanqueCurrent(?)) as alerta;"
			connection.query(selTempProgQry, getTempParams, function (err, results, fields) {
				...
				if (progTemp == 0)
					console.log("No hay valor de temperatura de referencia en el perfil!!");
				if (!error && progTemp > 0) {
					//los obj.r estan al reves, porque los relays funcionan al reves...
					if (tankTemp > tempRef) {
						if (obj.r == 1) {
		...
		updStatus = updStatus.concat((obj.r == 1 ? 0 : 1), obj.f);
